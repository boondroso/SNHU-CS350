...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        # Updates LEDs immediately when entering heat
        self.updateLights()
        if(DEBUG):
    ...
    def on_exit_heat(self):
        # Ensure the red LED is not left pulsing when leaving heat
        redLight.off()
    ...
    def on_enter_cool(self):
        # Update LEDs immediately when entering cool
        self.updateLights()
        if(DEBUG):
    ...
    def on_exit_cool(self):
        # Ensure the blue LED is not left pulsing when leaving cool
        blueLight.off()
    ...
    def processTempIncButton(self):
        ...
        # Bump setpoint and refresh indicators
        self.setPoint += 1
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        # Lower setpoint and refresh indicators
        self.setPoint -= 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        if(DEBUG):
            ...
            # HEAT: if temp < setPoint -> red LED fade; else solid red
            if self.current_state.id == "heat":
                if temp < self.setPoint:
                    # Smooth fade to indicate "calling for heat"
                    redLight.pulse(fade_in_time=0.8, fade_out_time=0.8, n=None, background=True)
                else:
                    redLight.on()
                blueLight.off()
            # COOL: if temp > setPoint -> blue LED fade; else solid blue
            elif self.current_state.id == "cool":
                if temp > self.setPoint:
                    # Smooth fade to indicate "calling for cooling"
                    blueLight.pulse(fade_in_time=0.8, fade_out_time=0.8, n=None, background=True)
                else:
                    blueLight.on()
                redLight.off()
            # OFF: both off
    ...
    def setupSerialOutput(self):
        # Format: state,temperature_F,setpoint_F + newline
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            # Example: "10/19 14:03:25   "
            lcd_line_1 = current_time.strftime("%m/%d %H:%M:%S")[:16].ljust(16)
    
            ## Setup Display Line 2
            if(altCounter < 6):
                temp_f = floor(self.getFahrenheit())
                lcd_line_2 = f"\nTemp: {temp_f}F".ljust(17)[:17]  # include leading newline
                altCounter = altCounter + 1
            else:
                # Show state and setpoint (e.g., "Heat SP:72F")
                state_name = self.current_state.id.capitalize()
                lcd_line_2 = f"\n{state_name} SP:{self.setPoint}F".ljust(17)[:17]
                altCounter = altCounter + 1
                if(altCounter >= 11):
...
greenButton.when_pressed = lambda: tsm.processTempStateButton()
...
redButton = Button(25)
# Increase setpoint
redButton.when_pressed = lambda: tsm.processTempIncButton()
...
# Decrease setpoint
blueButton.when_pressed = lambda: tsm.processTempDecButton()
